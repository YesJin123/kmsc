...
public class DemoComtroller {
	...
	public String parameterTestPage() {
		// /WEB-INF/views/ .jsp -> /WEB-INF/views/demo/demo1.jsp
		return "demo/demo";
	}
	
	//클라이언트가 보낸 데이터는 매핑메소드의 매개변수를 선언해서 처리함
	//1. HttpServletRequest : 요청정보를 저장하고 있는 객체
	//2. HttpServletReseponse : 응답 정보를 저장하고 있는 객체 -> 응답처리
	//3. HttpSession : Session 정보를 저장하는 객체(로그인 처리할 때)
	//4. java.utill.Locale : 서버의 로케일 정보를 저장하는 객체 -> 지역에 대한 정보 ko_KR
	//5. InputStream/Reader : 데이터를 서버로 읽어오는 객체 -> read()
	//6. OutputStream/Writer : 데이터를 서버에서 클라이언트에 보내는 객체 -> write()
	//7. 기본자료형 변수(int, double, String) : 클라이언트가 보낸 데이터 name값과 일치하게 선언
	//										-> 자동으로 저장시켜줌
	//8. VO클래스타입 : 클라이언트가 보낸 다수의 데이터를 저장하는 객체
	//				-> 필드명하고 name값이 일치할 때 자동 저장
	//* Date, 다른 클래스들.. spring이 처리하지 못하는 것은 403 BadRequest 에러를 발생!
	//9. java.utill.Map : 클라이언트가 보낸 데이터를 key : value 형식으로 데이터를 저장해줌!
	//* 주의! 단일값으로 저장 -> 복수값은 해결이 안됨!
	//매개변수 앞에 사용하는 어노테이션
	//@RequestParam : 클라이언트가 보낸 데이터를 저장할 때 추가 옵션 설정을 할 수 있는 어노테이션
	//@RequestBody : 클라이언트가 보낸 데이터가 JSON 방식일 때 변경해서 저장해주는 어노테이션
	...
	public String serlvetparamTest(HttpServletRequest request,
...
//		System.out.println(session);
		...
		//String[]을 반환
		String[] hoddy = request.getParameterValues("devLang");
		System.out.println(Arrays.toString(hoddy));
